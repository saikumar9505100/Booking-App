...
sh '''
sonar-scanner \
-Dsonar.projectKey=bank-service \
-Dsonar.sources=.
'''
}
}
...
--scan . --format HTML
--out dependency-check-report --prettyPrint
...
stage('Trivy Scan') {
steps {
sh 'trivy image --exit-code 0 --severity HIGH,CRITICAL ${ECR_URI}:${DOCKER_TAG}'
}
}
stage('ECR Push') {
steps {
...
post {
always { cleanWs() }
success { echo 'Bank Service Deployed!' }
